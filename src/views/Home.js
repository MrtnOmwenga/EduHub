...
const Home = () => {
  ...
  const demo = async () => {
    try {
      ...
      navigate('/studentsdashboard', { replace: true, state: { ...user, UserType: 'Student' } });
    } catch (exception) {
      ...
      <section className={style.Section1}>
        <nav className={style.navigation}>
          <div className={style.navbutton}>
            <p>
              ...
              |
              {' '}
              <button type="button" onClick={demo} className={style.alttext}>DEMO</button>
            </p>
          </div>
        </nav>
        <div className={style.container}>
          <div className={style.textcontainer}>
            ...
            <h5 className={style.text1}> SELF PACED ON DEMAND LEARNING ANYWHERE ANY TIME </h5>
            <Link to="/register">
